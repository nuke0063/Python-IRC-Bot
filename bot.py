...
# Commands from user:
        ...
        # Making this a subroutine would require nested while loops, which
        # ended up bad.
        if stream.find("$backup") != -1:
            try:
                # parts: [':User@champlain.edu', 'PRIVMSG', '#rawr',
                # ':$backup', 'source', 'dest']
                parts = stream.split()
                source = parts[4]
                dest = parts[5]
                ...
                sendmessage(
                    channel, "Invalid syntax, $backup /source/directory/ /dest/directory/")
                print parts
            else:
                info = str(commands.getstatusoutput(
                    "cp -r -u -x " + source + " " + dest))
        ...
        if stream.find("$command") != -1:  # Same as backup
            ...
            try:
                ...
                sendmessage(
                    channel, "Invalid syntax, usage: $command <command> [arg]")
                print parts
            else:
